#!/usr/bin/env python3
"""Validate the four-record claim-level provenance pilot.

This intentionally validates the sidecar pilot without changing production bills.json.
It checks structural invariants that the F-gate requires. Corpus migration should not
begin until this pilot and a rendered bill-detail page have been reviewed.
"""
...
def main() -> int:
    ...
    bills = load(ROOT / "registry" / "bills.json")
    ...
    for rec in pilot.get("records", []):
        ...
        if rid in seen_records:
            fail(errors, f"duplicate pilot record: {rid}")
        seen_records.add(rid)
        if rid not in PILOT_RECORDS:
            fail(errors, f"unexpected pilot record: {rid}")
        ...
        for i, entry in enumerate(rec.get("claims", [])):
            ...
            if mode == "derived":
                ...
                if not isinstance(supports, list) or len(supports) < 2:
                    fail(errors, f"{prefix}: derived claim requires at least two supports in this pilot")
            ...
            if rid in bill_by_id and claim.get("field") in {"codified_at", "effective_date"}:
                ...
                if actual != entry.get("value"):
                    fail(errors, f"{prefix}: stale value; bills.json has {actual!r}, pilot has {entry.get('value')!r}")
            if rid in bill_by_id and claim.get("field") == "status.stage":
                actual = (bill_by_id[rid].get("status") or {}).get("stage")
                if actual != entry.get("value"):
                    fail(errors, f"{prefix}: stale status.stage; bills.json has {actual!r}, pilot has {entry.get('value')!r}")

    if seen_records != PILOT_RECORDS:
        missing = sorted(PILOT_RECORDS - seen_records)
        fail(errors, f"pilot record set incomplete; missing {missing}")

    if errors:
        print("CLAIM PROVENANCE PILOT: FAIL")
        for error in errors:
            print(f"- {error}")
        return 1

    print("CLAIM PROVENANCE PILOT: PASS")
